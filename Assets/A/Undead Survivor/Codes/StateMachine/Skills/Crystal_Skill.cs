...
public class Crystal_Skill : Skill
{
   ...
   [SerializeField] private List<GameObject> crystalLeft = new List<GameObject>();
    ...
    public override void UseSkill()
    {
        ...
        {
            if(canMoveToEnemy)//움직이는 발사일떄는 캐릭터 위치이동 작동안하게
            return;
    ...
    private bool CanUseMultiCrystal()
    {
        if(canUseMultiStacks)
        {
            if(crystalLeft.Count > 0)
            {
                ...
                cooldown = 0;
                GameObject crystalToSpawn = crystalLeft[crystalLeft.Count-1];//리스트 중에 남은거중에 마지막꺼 가져오기
    ...
    {
        int amountToAdd = amountOfStacks - crystalLeft.Count;//최대값에서 남은 칸 만큼만 리필하도록
